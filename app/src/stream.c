...
static void
stream_close_sinks(struct stream *stream, unsigned count) {
    while (count) {
...
static inline void
stream_close_all_sinks(struct stream *stream) {
    stream_close_sinks(stream, stream->sink_count);
}
